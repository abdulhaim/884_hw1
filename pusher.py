...
class PusherEnv:
	...
	def compute_reward_push(self, state):
		# Reward is a function of distance to goal and angle between arm and object 

		arm_state = state[:3]
		obj_state = state[3:]

		# Distance from arm to obj & obj to goal 
		total_distance =  np.sqrt(np.sum((obj_state - arm_state)**2)) +  np.sqrt(np.sum((obj_state - self.goal)**2))

		if total_distance < 0.05:
			reward = 100 
		else: 
			reward = -100*(total_distance-1)
