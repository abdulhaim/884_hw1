...
class ReacherWallEnv:
	def __init__(self, action_repeat=10, render=False):
		...
		self.base_pos = (0.5*self.goal + 0.5*self.init)

		self.wall_pos = self.base_pos - np.array([0.05, 0, 0]) - [0.10,0.01,0]
	...
	def compute_reward_reach_wall(self, state):
		...
		## Old Reward 
		# distance_to_goal = np.sqrt(np.sum((state - self.goal)**2))
		# if distance_to_goal < 0.075:
		# 	reward = 100 
		# else: 
		# 	reward = -100*(distance_to_goal-1)
		# return reward

		## New Reward 
		if state[1] < self.wall_pos[1]: 
			state_to_wall = np.sqrt(np.sum((state - self.wall_pos)**2))
			wall_to_goal = np.sqrt(np.sum((self.wall_pos - self.goal)**2))
			reward_1 = -70*(state_to_wall-1)
			reward_2 = -30*(wall_to_goal-1)
			return reward_1 + reward_2

		else: 
			wall_to_goal = np.sqrt(np.sum((state - self.goal)**2))
			reward_3 = -100*(wall_to_goal-1)

			if wall_to_goal < 0.075:
				reward_3 = 100 
